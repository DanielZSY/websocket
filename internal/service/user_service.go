...
func (u *userService) GetUserList(uuid string) []model.User {
	...
	db.Raw("SELECT u.username, u.uuid, u.avatar FROM `im_user_friends` AS uf JOIN `im_users` AS u ON uf.friend_id = u.id WHERE uf.user_id = ?", queryUser.Id).Scan(&queryUsers)
