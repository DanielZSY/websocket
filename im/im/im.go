...
func GetGroupMessageDeliver(group_id int64) *group.GroupMessageDeliver {
	if group_id < 0 {
		group_id = -group_id
	}
...
// 群消息通知(apns, gcm...)
func PushGroupMessage(appid int64, group *group.Group, m *Message) {
...
func PublishMessage(appid int64, uid int64, msg *Message) {
	...
	amsg := &route.RouteMessage{appid: appid, receiver: uid, timestamp: now, msg: msg_buf}
	if msg.meta != nil {
...
func PublishGroupMessage(appid int64, group_id int64, msg *Message) {
	...
	amsg := &route.RouteMessage{appid: appid, receiver: group_id, timestamp: now, msg: msg_buf}
	if msg.meta != nil {
		meta := msg.meta.(*Metadata)
		amsg.msgid = meta.sync_key
		amsg.prev_msgid = meta.prev_sync_key
...
func SendAppGroupMessage(appid int64, group *group.Group, msg *Message) {
...
func DispatchAppMessage(amsg *route.RouteMessage) {
	now := time.Now().UnixNano()
...
func DispatchRoomMessage(amsg *route.RouteMessage) {
	mbuffer := bytes.NewBuffer(amsg.msg)
	msg := ReceiveMessage(mbuffer)
	if msg == nil {
...
func DispatchGroupMessage(amsg *route.RouteMessage) {
	now := time.Now().UnixNano()
	d := now - amsg.timestamp
	mbuffer := bytes.NewBuffer(amsg.msg)
	msg := ReceiveMessage(mbuffer)
...
func DispatchMessageToGroup(msg *Message, group *group.Group, appid int64, client *Client) bool {
	if group == nil {
		return false
	}
