...
var group_manager *group.GroupManager
...
var group_message_delivers []*group.GroupMessageDeliver
...
func main() {
	...
	config = config2.read_cfg(flag.Args()[0])
	...
	if len(config.mysqldb_datasource) > 0 {
		group_manager = group.NewGroupManager()
		group_manager.Start()
	}

	group_message_delivers = make([]*group.GroupMessageDeliver, config.group_deliver_count)
	for i := 0; i < config.group_deliver_count; i++ {
		q := fmt.Sprintf("q%d", i)
		r := path.Join(config.pending_root, q)
		deliver := group.NewGroupMessageDeliver(r)
		deliver.Start()
