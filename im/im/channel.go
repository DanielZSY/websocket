...
type Channel struct {
	...
	dispatch       func(*route.RouteMessage)
	dispatch_group func(*route.RouteMessage)
	dispatch_room  func(*route.RouteMessage)
}

func NewChannel(addr string, f func(*route.RouteMessage),
	f2 func(*route.RouteMessage), f3 func(*route.RouteMessage)) *Channel {
	channel := new(Channel)
...
func (channel *Channel) Subscribe(appid int64, uid int64, online bool) {
	...
	if count == 0 {
		...
		id := &route.SubscribeMessage{appid: appid, uid: uid, online: int8(on)}
		msg := &Message{cmd: MSG_SUBSCRIBE, body: id}
		channel.wt <- msg
	} else if online_count == 0 && online {
		//手机端上线
		id := &route.SubscribeMessage{appid: appid, uid: uid, online: 1}
		msg := &Message{cmd: MSG_SUBSCRIBE, body: id}
		channel.wt <- msg
	}
}
...
func (channel *Channel) Unsubscribe(appid int64, uid int64, online bool) {
	...
	if count == 1 {
		//用户断开全部连接
		id := &route.RouteUserID{appid: appid, uid: uid}
		msg := &Message{cmd: MSG_UNSUBSCRIBE, body: id}
		channel.wt <- msg
	} else if count > 1 && online_count == 1 && online {
		//手机端断开连接,pc/web端还未断开连接
		id := &route.SubscribeMessage{appid: appid, uid: uid, online: 0}
		msg := &Message{cmd: MSG_SUBSCRIBE, body: id}
		channel.wt <- msg
	}
}

func (channel *Channel) Publish(amsg *route.RouteMessage) {
	msg := &Message{cmd: MSG_PUBLISH, body: amsg}
	channel.wt <- msg
}

func (channel *Channel) PublishGroup(amsg *route.RouteMessage) {
	msg := &Message{cmd: MSG_PUBLISH_GROUP, body: amsg}
	channel.wt <- msg
}

func (channel *Channel) Push(appid int64, receivers []int64, msg *Message) {
	p := &route.BatchPushMessage{appid: appid, receivers: receivers, msg: msg}
...
func (channel *Channel) GetAllRoomSubscriber() []*route.RouteRoomID {
	channel.mutex.Lock()
	defer channel.mutex.Unlock()

	subs := make([]*route.RouteRoomID, 0, 100)
	for appid, s := range channel.subscribers {
		for room_id, _ := range s.room_ids {
			id := &route.RouteRoomID{appid: appid, room_id: room_id}
			subs = append(subs, id)
		}
	}
	return subs
...
func (channel *Channel) PublishRoom(amsg *route.RouteMessage) {
	...
	go func() {
		for {
			...
			if msg.cmd == MSG_PUBLISH {
				amsg := msg.body.(*route.RouteMessage)
