...
func newRPCClient(addr string) *im.RPCStorage {
	rpc_storage := im.NewRPCStorage([]string{addr}, nil)
	return rpc_storage
}

func Test_SetAndEnqueue() {
	dc := newRPCClient(storage_address)
	im := &im.IMMessage{sender: 1, receiver: 1000, content: "1111"}
	m := &im.Message{cmd: im.MSG_IM, body: im}
