...
func recv(uid int64, room_id int64, conn *net.TCPConn, count int, concurrent int) {
	...
	for received_num < n {
		...
		if d > time.Duration(3*60)*time.Second {
			seq++
			p := &im.Message{cmd: im.MSG_PING, seq: seq, version: im.DEFAULT_VERSION, flag: 0}
			err := im.SendMessage(conn, p)
			if err != nil {
		...
		msg, err := im.ReceiveServerMessage(conn)
		if err != nil {
...
func send(uid int64, room_id int64, conn *net.TCPConn, count int, concurrent int) {
	...
	for i := 0; i < count; i++ {
		...
		seq++
		msg := &im.Message{cmd: im.MSG_ROOM_IM, seq: seq, version: im.DEFAULT_VERSION, flag: 0,
			body: &im.RoomMessage{&im.RTMessage{uid, room_id, content}}}
...
func main() {
	...
	for i := 0; i < recv_count+concurrent; i++ {
		...
		var room_body im.Room = im.Room(room_id)
		m := &im.Message{cmd: im.MSG_ENTER_ROOM, seq: 2, version: im.DEFAULT_VERSION, flag: 0, body: &room_body}
