...
func send(uid int64, receiver int64, sem chan int) {
	...
	for i := 0; i < count; i++ {
		...
		seq++
		msg := &im.Message{cmd: im.MSG_IM, seq: seq, version: im.DEFAULT_VERSION, flag: 0,
			body: &im.IMMessage{uid, receiver, 0, int32(i), content}}
		...
		var ack *im.Message
		for {
			mm := im.ReceiveMessage(conn)
			if mm == nil {

				break
			}
			if mm.cmd == im.MSG_ACK {
				ack = mm
				break
			}
		}
...
func receive(uid int64, limit int, sem chan int) {
	...
	seq := 1
	auth := &im.AuthenticationToken{token: token, platform_id: 1, device_id: "00000000"}
	im.SendMessage(conn, &im.Message{cmd: im.MSG_AUTH_TOKEN, seq: seq, version: im.DEFAULT_VERSION, flag: 0, body: auth})
	im.ReceiveMessage(conn)

	seq++
	ss := &im.Message{cmd: im.MSG_SYNC, seq: seq, version: im.DEFAULT_VERSION, flag: 0, body: &im.SyncKey{sync_key}}
	im.SendMessage(conn, ss)
	...
	for {
		...
		msg := im.ReceiveMessage(conn)
		if msg == nil {
			log.Println("receive nill message")
			break
		}

		if msg.cmd == im.MSG_SYNC_NOTIFY {
			if !syncing {
				seq++
				s := &im.Message{cmd: im.MSG_SYNC, seq: seq, version: im.DEFAULT_VERSION, flag: 0, body: &im.SyncKey{sync_key}}
				im.SendMessage(conn, s)
				syncing = true
			} else {
				pending_sync = true
			}
		} else if msg.cmd == im.MSG_IM {
			...
			seq++
			ack := &im.Message{cmd: im.MSG_ACK, seq: seq, version: im.DEFAULT_VERSION, flag: 0, body: &im.MessageACK{seq: int32(msg.seq)}}
			im.SendMessage(conn, ack)
		} else if msg.cmd == im.MSG_SYNC_BEGIN {
			sync_count = 0
			//log.Println("sync begin:", recv_count)
		} else if msg.cmd == im.MSG_SYNC_END {
			syncing = false
			s := msg.body.(*im.SyncKey)
			//log.Println("sync end:", recv_count, s.sync_key, sync_key)
			if s.sync_key > sync_key {
				sync_key = s.sync_key
				//log.Println("sync key:", sync_key)
				seq++
				sk := &im.Message{cmd: im.MSG_SYNC_KEY, seq: seq, version: im.DEFAULT_VERSION, flag: 0, body: &im.SyncKey{sync_key}}
				im.SendMessage(conn, sk)
			}
			...
			log.Println("mmmmmm:", im.Command(msg.cmd))
		}
	}
	conn.Close()
	c <- true
