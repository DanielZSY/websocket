...
type Route struct {
	appid        int64
	mutex        sync.Mutex
	clients      map[int64]im.ClientSet
	room_clients map[int64]im.ClientSet
}

func NewRoute(appid int64) *Route {
	route := new(Route)
	route.appid = appid
	route.clients = make(map[int64]im.ClientSet)
	route.room_clients = make(map[int64]im.ClientSet)
	return route
}
...
func (route *Route) AddRoomClient(room_id int64, client *im.Client) {
	route.mutex.Lock()
	defer route.mutex.Unlock()
...
func (route *Route) FindRoomClientSet(room_id int64) im.ClientSet {
	route.mutex.Lock()
	defer route.mutex.Unlock()
...
func (route *Route) RemoveRoomClient(room_id int64, client *im.Client) bool {
	route.mutex.Lock()
	defer route.mutex.Unlock()
...
func (route *Route) AddClient(client *im.Client) bool {
	is_new := false
	route.mutex.Lock()
	defer route.mutex.Unlock()
...
func (route *Route) RemoveClient(client *im.Client) (bool, bool) {
	route.mutex.Lock()
	defer route.mutex.Unlock()
...
func (route *Route) FindClientSet(uid int64) im.ClientSet {
	route.mutex.Lock()
	defer route.mutex.Unlock()
...
func (route *Route) GetUserIDs() im.IntSet {
	return im.NewIntSet()
}
