...
type GroupMessageDeliver struct {
	...
	callbacks           map[int64]chan *im.Metadata //返回保存到ims的消息id
...
func NewGroupMessageDeliver(root string) *GroupMessageDeliver {
	...
	storage.callbacks = make(map[int64]chan *im.Metadata)
...
func (storage *GroupMessageDeliver) ReadMessage(file *os.File) *im.Message {
	//校验消息起始位置的magic
	var magic int32
	...
	msg := im.ReceiveStorageMessage(file)
	if msg == nil {
		return msg
	}
...
func (storage *GroupMessageDeliver) LoadMessage(msg_id int64) *im.Message {
	storage.mutex.Lock()
...
// save without lock
func (storage *GroupMessageDeliver) saveMessage(msg *im.Message) int64 {
	...
	body := msg.ToData()
	var msg_len int32 = im.MSG_HEADER_SIZE + int32(len(body))
	binary.Write(buffer, binary.BigEndian, msg_len)

	im.WriteHeader(int32(len(body)), int32(msg.seq), byte(msg.cmd),
	...
	log.Info("save message:", im.Command(msg.cmd), " ", msgid)
	return msgid
...
func (storage *GroupMessageDeliver) DoCallback(msgid int64, meta *im.Metadata) {
	storage.cb_mutex.Lock()
	defer storage.cb_mutex.Unlock()
...
func (storage *GroupMessageDeliver) AddCallback(msgid int64, ch chan *im.Metadata) int64 {
	storage.cb_mutex.Lock()
	defer storage.cb_mutex.Unlock()
	storage.id += 1
...
func (storage *GroupMessageDeliver) SaveMessage(msg *im.Message, ch chan *im.Metadata) int64 {
	storage.mutex.Lock()
	defer storage.mutex.Unlock()
...
// device_ID 发送者的设备ID
func (storage *GroupMessageDeliver) sendMessage(appid int64, uid int64, sender int64, device_ID int64, msg *im.Message) bool {

	im.PublishMessage(appid, uid, msg)

	route := im.app_route.FindRoute(appid)
	if route == nil {
		log.Warningf("can't send message, appid:%d uid:%d cmd:%s", appid, uid, im.Command(msg.cmd))
		return false
	}
...
func (storage *GroupMessageDeliver) sendGroupMessage(gm *im.PendingGroupMessage) (*im.Metadata, bool) {
	msg := &im.IMMessage{sender: gm.sender, receiver: gm.gid, timestamp: gm.timestamp, content: gm.content}
	...
	for _, member := range gm.members {
		index := im.rpc_storage.GetStorageRPCIndex(member)
	...
	for _, mb := range batch_members {
		r, err := im.rpc_storage.SavePeerGroupMessage(gm.appid, mb, gm.device_ID, m)
		if err != nil {
		...
		for i := 0; i < len(r); i++ {
			...
			mm := &im.Message{cmd: im.MSG_GROUP_IM, version: im.DEFAULT_VERSION,
				flag: im.MESSAGE_FLAG_PUSH, body: msg, meta: meta}
			storage.sendMessage(gm.appid, member, gm.sender, gm.device_ID, mm)

			notify := &im.Message{cmd: im.MSG_SYNC_NOTIFY, body: &im.SyncKey{msgid}}
	...
	im.PushGroupMessage(gm.appid, group, m)
...
func (storage *GroupMessageDeliver) sendPendingMessage() {
	...
	for {
		...
		if msg.cmd != im.MSG_PENDING_GROUP_MESSAGE {
			continue
		}

		gm := msg.body.(*im.PendingGroupMessage)
...
func (storage *GroupMessageDeliver) LoadGroup(gid int64) *Group {
	group := im.group_manager.FindGroup(gid)
	if group != nil {
		return group
	}
...
func (storage *GroupMessageDeliver) DispatchMessage(msg *im.Message, group_id int64, appid int64) {
	//assert(msg.appid > 0)
	group := im.group_manager.FindGroup(group_id)
	if group != nil {
		im.DispatchMessageToGroup(msg, group, appid, nil)
	} else {
		select {
...
func (storage *GroupMessageDeliver) dispatchMessage(msg *im.Message, group_id int64, appid int64) {
	group := im.group_manager.LoadGroup(group_id)
	if group == nil {
...
func (storage *GroupMessageDeliver) loadGroup(gl *GroupLoader) {
	group := im.group_manager.LoadGroup(gl.gid)
	gl.c <- group
}
