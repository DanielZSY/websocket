...
type AppRoute struct {
	mutex sync.Mutex
	apps  map[int64]*route.Route
}

func NewAppRoute() *AppRoute {
	app_route := new(AppRoute)
	app_route.apps = make(map[int64]*route.Route)
	return app_route
}

func (app_route *AppRoute) FindOrAddRoute(appid int64) *route.Route {
	app_route.mutex.Lock()
	defer app_route.mutex.Unlock()
	if route, ok := app_route.apps[appid]; ok {
		return route
	}
	route := route.NewRoute(appid)
	app_route.apps[appid] = route
	return route
}

func (app_route *AppRoute) FindRoute(appid int64) *route.Route {
	app_route.mutex.Lock()
	defer app_route.mutex.Unlock()
	return app_route.apps[appid]
}

func (app_route *AppRoute) AddRoute(route *route.Route) {
	app_route.mutex.Lock()
	defer app_route.mutex.Unlock()
